...
class UserStore {
    ...
    // آسیب‌پذیری پنهان: مدیریت کش داخلی
    struct CacheEntry {
        char username[48];
        char role[24];
        time_t timestamp;
...
public:
    ...
    void load() {
        ...
        while (std::getline(ifs, line)) {
            ...
            if (opt) {
                ...
                // آسیب‌پذیری: اضافه کردن به کش با کپی ناامن
                CacheEntry entry;
    ...
    void add_or_update(const User& u) {
        ...
        // به‌روزرسانی کش با آسیب‌پذیری پنهان
    ...
    std::string get_user_summary(const std::string& username) const {
        ...
        if (user) {
            // آسیب‌پذیری: format string با بافر ثابت
...
class AuthManager {
    ...
    public:
        void log_event(const std::string& event_type, const std::string& username, const std::string& details) {
            // آسیب‌پذیری: ساخت لاگ با چندین مرحله
            char timestamp[32];
            std::time_t now = std::time(nullptr);
            std::strftime(timestamp, sizeof(timestamp), "%Y-%m-%d %H:%M:%S", std::localtime(&now));
            
            // مرحله اول: ساخت رشته جزئیات
            snprintf(temp_buffer, sizeof(temp_buffer), "Details: %s", details.c_str());
            
            // مرحله دوم: ساخت لاگ نهایی - آسیب‌پذیری اصلی اینجا است
...
public:
    ...
    // admin utility (requires to be called by trusted code)
...
int main() {
    ...
    // آسیب‌پذیری پنهان: مدیریت وضعیت برنامه
    struct AppState {
    ...
    while (true) {
        ...
        // به‌روزرسانی وضعیت برنامه با آسیب‌پذیری پنهان
        ...
        if (cmd == "register") {
            ...
            // آسیب‌پذیری: به‌روزرسانی current_user
